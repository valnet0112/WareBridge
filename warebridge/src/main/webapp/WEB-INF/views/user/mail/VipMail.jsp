...
<div id="inner-card-body" class="card-body" style="height: 82vh">
	<h4 class="card-header m-0 p-0">중요 메일함</h4>
	<hr>
		...
		<table class="table table-hover">
			...
			<c:forEach items="${mailList }" var="mail">
				<tr class="mail-inside" data-mailcode="${mail.mailCode }">
					<td>
				    	<c:choose>
					        <c:when test="${mail.mailVip eq 'Y'}">
								<input class="mailCheck form-check-input" type="checkbox" data-mail-code = "${mail.mailCode }">
								<i class="light-style cursor-pointer tf-icons bx bx-bulb bx-sm"></i>
							</c:when>
					        <c:otherwise>
					            <input class="mailCheck form-check-input" type="checkbox" data-mail-code = "${mail.mailCode }">
								<i class="cursor-pointer tf-icons bx bx-bulb bx-sm"></i>
					        </c:otherwise>
					    </c:choose>
					</td>
